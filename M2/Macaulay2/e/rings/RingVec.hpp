  ...
  while (v != NULL)
    {
      ...
      if (v->comp < r) return crtp()->from_long(0);
      v = v->next;
    }
  return crtp()->from_long(0);
}
...
vec RingBase<D>::negate_vec(vec v) const
{
  ...
  for (vecterm *a = v; a != NULL; a = a->next)
    {
      b->next = make_vec(a->comp, crtp()->negate(a->coeff));
      b = b->next;
    }
  b->next = NULL;
  return result.next;
...
{
  if (crtp()->is_zero(f)) return NULL;
  vecterm head;
  vec result = &head;
  for (vec v = w; v != 0; v = v->next)
    {
      ring_elem a = crtp()->mult(f, v->coeff);
      if (!crtp()->is_zero(a))
        {
...
vec RingBase<D>::rightmult_vec(const vec w, const ring_elem f) const
{
  ...
  for (vec v = w; v != 0; v = v->next)
    {
      ring_elem a = crtp()->mult(v->coeff, f);
      if (!crtp()->is_zero(a))
        {
          vec t = make_vec(v->comp, a);
          result->next = t;
...
template <typename D>
vec RingBase<D>::tensor(const FreeModule *F, vec v, const FreeModule *G, vec w) const
{
  // FIXME(RZ): for compilation testing only
  return new_vec();
...
template <typename D>
vec RingBase<D>::vec_eval(const RingMap *map, const FreeModule *F, const vec v) const
// v is a vector over 'this'
{
  // FIXME(RZ): for testing purposes only
  return new_vec();

//  const D *targetRing = map->get_ring();
//
//  vecterm head;
//  vec result = &head;
//
...
//    {
//      ring_elem a = crtp()->eval(map, t->coeff, 0);  // a is now in the target ring
...
vec RingBase<D>::vec_zeroize_tiny(gmp_RR epsilon, const vec v) const
{
  ...
  for (const vecterm *p = v; p != 0; p = p->next)
    {
      ring_elem a = crtp()->zeroize_tiny(epsilon, p->coeff);
      if (!crtp()->is_zero(a))
        {
          vec w = new_vec();
          result->next = w;
...
{
  for (const vecterm *p = v; p != 0; p = p->next)
    crtp()->increase_maxnorm(norm, p->coeff);
}
///////////////////////////////////////
  ...
  for (vec p = &head; p->next != 0; p = p->next)
    if (p->next->comp < i)
    ...
    else if (p->next->comp == i)
      {
        ring_elem c;
        if (opposite_mult)
          c = crtp()->mult(p->next->coeff, r);
        else
          c = crtp()->mult(r, p->next->coeff);
        p->next->coeff = c;
        if (this->is_zero(p->next->coeff))
          {
            vec tmp = p->next;
...
template <typename D>
vec RingBase<D>::mult_vec_matrix(const Matrix *m, vec v, bool opposite_mult) const
{
...
template <typename D>
vec RingBase<D>::vec_diff(vec v, int rankFw, vec w, int use_coeff) const
// rankFw is the rank of the free module corresponding to w.
{
  ...
  for (; v != NULL; v = v->next)
    for (vecterm *p = w; p != NULL; p = p->next)
      {
        ring_elem a = crtp()->diff(v->coeff, p->coeff, use_coeff);
        if (crtp()->is_zero(a))
          {
            crtp()->remove(a);
            continue;
...
template <typename D>
int RingBase<D>::vec_in_subring(int nslots, const vec v) const
{
  const PolynomialRing *PR = crtp()->cast_to_PolynomialRing();
...
template <typename D>
void RingBase<D>::vec_degree_of_var(int n, const vec v, int &lo, int &hi) const
{
  if (v == NULL)
    {
  ...
  crtp()->degree_of_var(n, v->coeff, lo, hi);
  for (vec w = v->next; w != 0; w = w->next)
    {
      int lo1, hi1;
      crtp()->degree_of_var(n, w->coeff, lo1, hi1);
      if (lo1 < lo) lo = lo1;
      if (hi1 > hi) hi = hi1;
    }
}
...
template <typename D>
vec RingBase<D>::vec_divide_by_var(int n, int d, const vec v) const
{
  vecterm head;
  vecterm *result = &head;
  for (vec w = v; w != 0; w = w->next)
    {
      ring_elem a = crtp()->divide_by_var(n, d, w->coeff);
      if (!crtp()->is_zero(a))
        {
...
template <typename D>
vec RingBase<D>::vec_divide_by_expvector(const int *exp, const vec v) const
{
  vecterm head;
  vecterm *result = &head;
  for (vec w = v; w != 0; w = w->next)
    {
      ring_elem a = crtp()->divide_by_expvector(exp, w->coeff);
      if (!crtp()->is_zero(a))
        {
          vec t = make_vec(w->comp, a);
          result->next = t;
                              ...
                              int &hi) const
{
  ...
  crtp()->degree_weights(t->coeff, wts, lo, hi);
  lo += F->primary_degree(t->comp);
  hi += F->primary_degree(t->comp);
  for (t = t->next; t != NULL; t = t->next)
    {
      int lo1, hi1;
      crtp()->degree_weights(t->coeff, wts, lo1, hi1);
  ...
  for (vec w = f; w != 0; w = w->next)
    {
      ...
      ring_elem a = crtp()->homogenize(w->coeff, v, d - e, wts);
      if (!crtp()->is_zero(a))
        {
...
{
  // FIXME(RZ): this is temporary! just to make sure everything compiles right
...
vec RingBase<D>::vec_remove_monomial_factors(vec f, bool make_squarefree_only) const
{
  ...
  for (vec a = f; a != NULL; a = a->next) crtp()->monomial_divisor(a->coeff, exp);
